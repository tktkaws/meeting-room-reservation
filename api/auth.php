...
function handleLogin() {
    ...
    try {
        $db = getDatabase();
        $stmt = $db->prepare('SELECT id, name, email, password, role, department_id FROM users WHERE email = ?');
        $stmt->execute([$email]);
        $user = $stmt->fetch();
    } catch (Exception $e) {
    ...
    $_SESSION['department_id'] = sanitizeInput($user['department_id'] ?? 1);
    ...
    sendJsonResponse(true, 'ログインしました', [
        'user' => [
            ...
            'department_id' => sanitizeInput($user['department_id'] ?? 1)
        ]
    ]);
}
...
function handleRegister() {
    ...
    $email = sanitizeInput($_POST['email'] ?? '');
    $password = $_POST['password'] ?? '';
    $department_id = sanitizeInput($_POST['department_id'] ?? 1);
    ...
    if (!validateInput($department_id, 'int')) {
        sendJsonResponse(false, '有効な部署を選択してください', null, 400);
    }
    ...
    // ユーザー登録
    $tagColors = json_encode(["1" => "#4299E1", "2" => "#48BB78", "3" => "#ED8936", "4" => "#9F7AEA", "5" => "#38B2AC"]);
    $stmt = $db->prepare('INSERT INTO users (name, email, password, department_id, tag_colors) VALUES (?, ?, ?, ?, ?)');
    
    try {
        $stmt->execute([$name, $email, $password, $department_id, $tagColors]);
